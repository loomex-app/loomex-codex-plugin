...
Use this reference only after an explicit live follow request. The
...
`loomex-recovery:<current-task-id>:<runId>`. A task following two runs
...
1. Resolve a host task identity through a supported host API. A workspace path,
   `codex-ui`, `unbound`, a hook session ID, a UI card, or a previous chat is
   not that identity. If none is available, remain live-only and do not make a
   recovery mutation.
2. Call `loomex_recovery_get` for `{hostId, hostTaskId, runId}`. If the local
...
3. After the first fresh active run read and event drain, call
...
4. Execute that exact host `automation_update` create, retain its returned
...
5. If the journal says `attempt_in_flight`, reconcile its current operation
...
Exact marker: loomex-recovery:<current-task-id>:<runId>.
