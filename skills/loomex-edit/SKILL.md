...
Resolve the existing workflow and read its current draft/version. Read [authoring guidance](../loomex-workflows/references/authoring.md) and use the editor lifecycle for conversational edits, preserving untouched behavior and concurrent-version checks. Preview the final update before applying it. Return the applied draft changes and validation result; do not silently publish or activate.
