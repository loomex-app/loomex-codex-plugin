...
The runner owns the browser callback and credential exchange. A Sign in click in the card starts a flow and shows its link in that card; the card cannot append an assistant message to the existing chat turn. It offers an Open browser action where supported and a copyable link as fallback. Never treat a link click as authentication completion. The runner's callback and a fresh connection read establish completion.
