...
Read [the visual delivery contract](../loomex-workflows/references/visual-delivery.md) for the single setup/preparation card and its headless fallback.

Resolve the requested workflow and version. Read [execution guidance](../loomex-workflows/references/execution.md) and follow task-workspace selection, setup, missing-input collection, exact preparation and review. Pass the actual local Codex task cwd to setup when available; preserve an explicit workspace override, model/provider and reasoning preferences, asking only for unresolved required choices. After accepted Start, read [monitoring guidance](../loomex-workflows/references/monitoring.md) and follow that exact run. If an existing run ID or monitor_existing_run continuation was supplied, use monitoring directly instead of creating a run. For preparation-only or no-monitor requests honor that narrower scope.
