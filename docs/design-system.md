...
The workflow browser uses compact divided rows within the shared typography, controls and responsive shell. Each row shows the name, optional description, status, published version and step count, followed by restrained `View` and `Prepare run` actions. Desktop rows keep metadata and actions on one line when space permits; narrow screens place two full-width 42px action targets below the copy. UUIDs remain action identifiers rather than primary display text. Search uses an explicit labelled form and pagination uses labelled navigation.

Workflow detail is a shared read view used after browser selection and for a direct `loomex_workflow_get` authoring resource. The selected immutable version definition is authoritative even when the active version differs, and direct-authoring refreshes request that same version number. The view shows authored description, status and version; input titles, types and required state; bounded AI model/provider/effort configuration; and the declared execution policy with workspace-selection semantics. It renders at most the first 50 input requirements and 20 unique AI configurations, and states the displayed and total counts when more are present. Steps remain collapsed, render at most the first 100 bounded names and types, and state omissions without exposing raw JSON or identifier walls. Browser details return to the retained list page. Both browser and direct authoring `Prepare run` actions send the same read-only conversation handoff for input, workspace and exact preparation review; they never commit or execute a run. Direct preparation stays disabled while a refresh is pending and after a failed authoritative refresh, then returns after a valid read. Paged direct-authoring responses use the same safe conversation retrieval as paged browser responses without issuing another workflow or mutation call.
