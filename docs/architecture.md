...
Run monitoring uses the same typed interaction renderer in both a newly started run card and a standalone monitor. Explicit answers and approval decisions call the existing interaction tools directly, then refresh that execution. Core human continuation does not depend on a `ui/message` host handoff. Model-facing guidance remains complete for headless use: inspect the exact run, fetch its pending interaction, collect the user’s answer, submit that answer, and inspect the same run again.
