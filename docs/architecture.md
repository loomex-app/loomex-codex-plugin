...
The plugin vendors exact copies in `contracts/` and records their SHA-256 digests in `contracts/contract-pin.json`. The current catalog contains 47 runner methods. The plugin exposes 46 as focused `loomex_*` MCP tools; `daemon.drain` is reserved for runner installation and operations and is intentionally omitted. Tests compare the exposed methods and top-level input/output fields to the pinned catalog and reject a hash mismatch.

The checked-in build script packages the plugin's vendored copies. It does not fetch or regenerate contracts and does not currently copy them from `../runner` during packaging. A protocol change therefore requires an explicit synchronized update and pin before either product is released. Product version `0.1.0` and protocol version `v2` are separate version axes.
...
Every mutation takes a UUID idempotency key. The plugin makes one local call and never automatically retries it. If the connection fails after a mutation may have been sent, it returns `NETWORK_AMBIGUOUS` with the retained key. Reconciliation must reuse that key for the same intended change. Read transport failures return `RUNNER_UNAVAILABLE`; the catalog's read retry disposition permits a caller retry but the plugin does not perform that retry internally.
