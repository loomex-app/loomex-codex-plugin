...
The catalog covers readiness, connection/authentication, organization selection, workspace grants, workflow authoring and publication, preparation and commit, run monitoring, complete results, events, interactions, cancellation, deletion, artifacts, and large-response paging. Conversational creation and editing use the active host chat to construct and validate definitions. Creation with a definition saves the workflow and its canonical draft atomically; edits use the current draft revision. These operations do not execute providers or need workspace approval. Provider choices inside a workflow describe its eventual execution, not the authoring assistant. Existing execution-backed builder/editor tools remain compatibility and recovery interfaces, not the conversational entrypoint. The intended connection surface names are `loomex_connection_get` for the headless state read and `loomex_connection_view` for the focused interactive view; the runner and UI worker remain authoritative for their exact availability and result shape.
