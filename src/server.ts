  ...
  const server = new McpServer(
    { name: "loomex", version: "0.9.0" },
    {
      ...
      instructions: [
        ...
        "Status reads once. Explicit $loomex-follow or monitor_existing_run continuation starts with loomex_run_get for that exact runId before any status reply, including when the host wraps it as contextual UI input. An accepted-interaction receipt invalidates the old pending-form assumption; verify current state instead of asking to submit that form again. Follow fresh nextAction: drain hasMoreEvents before advancing sequence, then use one loomex_run_wait at a time with timeoutSeconds 30. Quiet timeouts are not completion. Report meaningful changes only. Stop for human input, terminal results, user stop or actionable errors; fetch terminal results and needed pages.",
