 ...
 */
export const MONITORING_CONTRACT_VERSION = "loomex-monitoring/v1";
 ...
 * only the runner can verify it. A bare command remains available for a human
 * explicitly requesting a follow, but generated text must carry the receipt.
 */
export const FOLLOW_CONTINUATION_SCHEMA_VERSION = "loomex.follow-session.continuation/v1" as const;
export const FOLLOW_COMMAND = "$loomex-follow" as const;
...
export type FollowContinuation = {
  ...
  source: "bare_command" | "generated_markdown";
  runId: string;
...
const FOLLOW_BARE_PATTERN = new RegExp(`^${escapeRegex(FOLLOW_COMMAND)} (${FOLLOW_RUN_ID})$`, "i");
const FOLLOW_GENERATED_PATTERN = new RegExp(
  `^${escapeRegex(FOLLOW_COMMAND)} (${FOLLOW_RUN_ID})\\n\\n<!-- loomex-follow-continuation/v1 receipt=(${FOLLOW_RECEIPT}) -->\\n\\n${FOLLOW_GENERATED_MARKDOWN_INSTRUCTIONS.map(escapeRegex).join("\\n\\n")}$`,
  "i",
);
...
export function formatFollowContinuationMarkdown(runId: string, receipt: string): string {
  ...
  return `${FOLLOW_COMMAND} ${runId}\n\n<!-- loomex-follow-continuation/v1 receipt=${receipt} -->\n\n${FOLLOW_GENERATED_MARKDOWN_INSTRUCTIONS.join("\n\n")}`;
}

/**
 * Render factual, model-visible continuation state separately from the
 * activatable follow command. A fenced JSON block keeps structured context
 * legible in chat transcripts without making it part of the command grammar.
 */
export function formatFollowContinuationContext(context: Record<string, unknown>): string {
  const serialized = JSON.stringify(context);
  if (typeof serialized !== "string" || serialized.length > 4096) {
    throw new TypeError("invalid Loomex follow continuation context");
  }
  return `Loomex continuation context:\n\n\`\`\`json\n${serialized}\n\`\`\``;
}
...
export function formatManualFollowInstruction(runId: string): string {
  ...
  return `To continue monitoring this exact Loomex run, submit \`${FOLLOW_COMMAND} ${runId}\`.`;
}

/**
 * Recognize only a bare command or the complete formatter output. In
 * particular, quoted, fenced, embedded, or edited continuations are inert.
 */
export function parseFollowContinuation(value: unknown): FollowContinuation | undefined {
  if (typeof value !== "string" || value.length > 4096) return undefined;
  const bare = value.match(FOLLOW_BARE_PATTERN);
  if (bare?.[1] !== undefined) {
    return { schemaVersion: FOLLOW_CONTINUATION_SCHEMA_VERSION, source: "bare_command", runId: bare[1].toLowerCase() };
  }
...
export interface MonitoringContract {
  ...
  recovery: {
    ...
    registrationState: RecoveryRegistrationState;
    observedLifecycle: RecoveryLifecycleState;
  ...
  /** nextAction remains the only authoritative runner action. */
  liveFollow: {
...
export function monitoringContract(options: MonitoringContractOptions): MonitoringContract {
  // A runner snapshot has no durable host history. Unknown history is
  // ambiguous and therefore cannot authorize a new create.
  const registrationState = options.recoveryRegistrationState ?? "ambiguous";
  const base = {
    registrationState,
  ...
  if (options.state === "active") {
    const requiredAction = registrationState === "not_attempted"
      ? "initialize_and_verify"
  ...
  if (options.state === "needs_input") {
    return {
      ...
      liveFollow: { mode: "stop", nextAction: "authoritative_runner_action" },
    };
  }
  if (options.state === "terminal") {
    return {
      contract: MONITORING_CONTRACT_VERSION,
      state: "terminal",
      observation: options.observation ?? (options.resultPending ? "terminal_result_pending" : "terminal_result_retrieved"),
      recovery: {
        ...base,
        requiredLifecycle: "removed",
        requiredAction: options.resultPending ? "retrieve_result_then_remove" : "remove",
        initialization: "not_applicable",
      },
      liveFollow: { mode: "stop", nextAction: "authoritative_runner_action" },
    };
...
export const MONITORING_MODEL_INSTRUCTIONS = [
  ...
  "While an exact run remains active, resolve supported host scheduling and the current task identity. Durable recovery registration is separate from host-observed lifecycle: not_attempted, attempt_in_flight, registered, ambiguous, and removed. On the first active phase after accepted initial human input, create only when the durable registration state is exactly not_attempted; attempt_in_flight and registered require reconciliation of their known record, and ambiguous or unknown history never authorizes a create. View a known record with the host schema's ID only, then verify the returned same-task, same-run record before calling recovery verified. Use the two-minute cadence. If task identity or host scheduling is unavailable, report recovery unavailable once and continue live polling.",
  "Use the durable journal as the registration authority: for a run-start continuation call loomex_recovery_get and initialize the local record with loomex_recovery_update even when the first run read is waiting for input; one-off status reads never do this. On the first active phase, get the record again. For not_attempted, update if needed, begin the exact host create with loomex_recovery_operation_begin, and create only after attemptPermitted is true; settle the returned automation ID with loomex_recovery_operation_settle, then verify with host automation view using its ID only. For attempt_in_flight or registered, reconcile the journal's known operation or ID. Unknown history remains ambiguous. If schedule creation or update is ambiguous, persist attempt_in_flight and view the known exact record before any retry. If no record ID is known, preserve ambiguous registration rather than creating a duplicate. Do not claim recovery is active without verification.",
  "For an explicit live follow, after initialization use one 30-second loomex_run_wait at a time. Quiet timeouts are active work, never completion or a reason to end the follow loop. A scheduled recovery turn reads once, drains pages, reports only a new actionable state, and does not enter an indefinite live loop.",
  "Before presenting human input or an actionable error, pause the exact recovery. After complete terminal result retrieval, remove it; if removal fails, pause it and report the cleanup dependency. User stop removes or pauses recovery and never cancels execution.",
...
export const RUN_GET_MONITORING_DESCRIPTION =
  "Read current authoritative state for one exact run. A one-off status request ends with this snapshot. An explicit follow, run-start continuation, or accepted-interaction continuation must fresh-read, drain event pages, then follow the durable recovery registration state and direct host record workflow independently of the live nextAction.";

export const RUN_WAIT_MONITORING_DESCRIPTION =
  "Wait for one bounded run change (currently at most 45 seconds). For an explicit live follow, first fresh-read and drain pages, then initialize or reconcile host recovery according to durable registration state before serial 30-second waits. A quiet timeout remains active work; scheduled recovery never starts an indefinite live loop.";
